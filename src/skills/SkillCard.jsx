...
export const SkillCard = ({ skills, title, title2 = "", marquee = false }) => {
  const renderSkills = () =>
    skills.map((skill) => (
      <div className="skill-item glass-card" key={skill.id} data-aos="fade-up">
        <img src={skill.url} alt={skill.name} className="skills-logo" />
        <p className="title-icon">{skill.name}</p>
      </div>
    ));
      ...
      {marquee ? (
        <div className="skills-marquee-wrapper">
          <Marquee
            ...
            pauseOnClick={false}
            direction="left">
            <div className="marquee-track">{renderSkills()}</div>
          </Marquee>
        </div>
      ) : (
